function [Mingood, Mingerber, Minyield] = calculate_torsion_fatigue( handles,electrode,steel,l,mforce,aforce,h,material,Area,Xav,Yav,Ju,Ra,Rb,Rc,Rd,Oa,Ob,Oc,Od )
...
Trma = ((Tm)^2 + (T2ma)^2 + 2*Tm*T2ma*cosd(Oa))^(1/2);
Trmb = ((Tm)^2 + (T2mb)^2 + 2*Tm*T2mb*cosd(Ob))^(1/2);
Trmc = ((Tm)^2 + (T2mc)^2 + 2*Tm*T2mc*cosd(Oc))^(1/2);
Trmd = ((Tm)^2 + (T2md)^2 + 2*Tm*T2md*cosd(Od))^(1/2);


Traa = ((Ta)^2 + (T2aa)^2 + 2*Ta*T2aa*cosd(Oa))^(1/2);
Trab = ((Ta)^2 + (T2ab)^2 + 2*Ta*T2ab*cosd(Ob))^(1/2);
Trac = ((Ta)^2 + (T2ac)^2 + 2*Ta*T2ac*cosd(Oc))^(1/2);
Trad = ((Ta)^2 + (T2ad)^2 + 2*Ta*T2ad*cosd(Od))^(1/2);
